...
def main():
	...
	### ---------------- Change the path of input and output FRAMES HERE -------------- ###
	### ---------------- Noted that the input and output are FRAMES, NOT VIDEOS ---------------- ###
        video_path = 'demo/Vid4_BI/' + name
        save_path =  'demo/Vid4_BI_results/' + name
        imgs_lq = load_images_to_tensor(video_path)
        model = IART(mid_channels=64,pth
                    embed_dim=120,
                    depths=[6, 6, 6],
                    num_heads=[6, 6, 6],
                    window_size=[3, 8, 8],
	...
	### ---- The baseline (Original IART) is located at "experiments/IART_REDS_N5_15K/models/net_g_latest.pth" ----###
	### ---- The improved one (ours) is at "experiments/IART_REDS_N5_15K_Improved_Dynamic/models/net_g_latest.pth" ) ----###
	### ---- The models with higher traning iteration are still on the way ----#
