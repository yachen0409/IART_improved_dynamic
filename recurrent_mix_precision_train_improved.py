...
# ### [MODIFICATION START]
def calculate_temporal_dissimilarity(lq_frames):
    """
    Calculates the average pixel difference between consecutive frames.
    """
    # lq_frames shape: [Batch, Frames, Channel, Height, Width]
    if lq_frames.size(1) < 2:
        return 0.5 # Neutral if only 1 frame
    
    # Calculate difference between Frame T and Frame T-1
    diff = torch.abs(lq_frames[:, 1:, :, :, :] - lq_frames[:, :-1, :, :, :])
    
    # Mean value of differences (The "Dissimilarity Measure" from Slide 5)
...
def train_pipeline(root_path):
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            data_timer.record()
            # We check the 'lq' (Low Quality) input for outliers or duplicates.
            if 'lq' in train_data:
                dissimilarity = calculate_temporal_dissimilarity(train_data['lq'])
                if dissimilarity > OUTLIER_THRESHOLD:
                    logger.warning(f"Skipping Iter {current_iter}: Detected Outlier/Scene Cut (Diff: {dissimilarity:.4f})")
                    train_data = prefetcher.next()
                    continue # Skip this batch
                if dissimilarity < DUPLICATE_THRESHOLD:
                    # We don't skip, but we log it to be aware of data redundancy
                    # Optionally: You could 'continue' here too if you want strict deduplication.
                    pass 
