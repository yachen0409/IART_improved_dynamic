...
class RecurrentMixPrecisionRTModel(VideoRecurrentModel):
    ...
    def feed_data(self, data):
        super().feed_data(data) # Let parent handle lq and gt
        # Capture the weight we calculated in the train script
        if 'manual_loss_weight' in data:
            self.manual_loss_weight = data['manual_loss_weight']
        else:
            self.manual_loss_weight = None
    # -------------------------------- [Modification End]
    ...
    def optimize_parameters(self, scaler, current_iter):
		...
		# --------------------------- [MODIFICATION] Apply Dynamic Weighting
                if getattr(self, 'manual_loss_weight', None) is not None:
                    l_pix = l_pix * self.manual_loss_weight
		# --------------------------- [MODIFICATION End]
