...
def calculate_temporal_dissimilarity(lq_frames):
    """
    Calculates the average pixel difference.
    Returns a Tensor scalar (on GPU) for efficient weighting.
    """
    # lq_frames shape: [Batch, Frames, Channel, Height, Width]
    if lq_frames.size(1) < 2:
        return torch.tensor(0.0, device=lq_frames.device)
    
    # Calculate difference
    diff = torch.abs(lq_frames[:, 1:, :, :, :] - lq_frames[:, :-1, :, :, :])
    
    # Mean value (Scalar for the whole batch)
        ...
        while train_data is not None:
 	    ...
 	    # ----------------------------- ### [MODIFICATION START]
            if 'lq' in train_data:
                ...
                batch_weight = torch.exp(-WEIGHT_DECAY_FACTOR * dissimilarity)
                batch_weight = torch.clamp(batch_weight, min=0.1, max=1.0)
                train_data['manual_loss_weight'] = batch_weight

                # Optional Logging
                if dissimilarity > 0.1 and current_iter % opt['logger']['print_freq'] == 0:
                     logger.info(f"Iter {current_iter}: High Dissim ({dissimilarity:.4f}) -> Weighting Loss by {batch_weight:.4f}")
            # ----------------------------- ### [MODIFICATION END]
